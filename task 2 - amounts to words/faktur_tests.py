...
class TestFaktur(unittest.TestCase):
    ...
    def test_example_values(self):
        for kwota, slownie in PRZYKLADY_KWOT.items():
            self.assertEqual(fiddler.fakturowanie(Decimal(kwota)), slownie)

    def test_raise_value_error(self):
        self.assertRaises(TypeError, fiddler.fakturowanie, 'a')
        self.assertRaises(TypeError, fiddler.fakturowanie, 2)
